...
func (s *service) VerifyCode(ctx context.Context, code string) (domain.WechatInfo, error) {
	...
	// 只读一遍
	// resp.Body 是io.ReadCloser类型 是interface 包含了Reader Closer两个interface
	decoder := json.NewDecoder(resp.Body) // NewDecoder接收io.Reader类型的数据 返回一个decoder decoder具有decode方法 即json转结构体
	var res Result
